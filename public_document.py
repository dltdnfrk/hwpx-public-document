...
class ManualDraftResult:
    ...
    hwp_export: HWPExportOutcome | None = None
...
def build_policy_plan_draft(
    ...
    *,
    ...
    for field in _GROUNDED_FIELDS | _PLAN_FIELDS:
...
def export_hwpx(draft: Draft, destination: Path, *, operation_id: str) -> Path:
    ...
    if draft.page_count > 5:
        raise PageLimitExceededError("HWPX export is limited to five pages")
    parts = {
...
def complete_manual_draft(
    ...
    *,
    ...
    if hwp_adapter is not None and hwp_destination is not None:
        try:
            hwp_export = HWPExportOutcome(
                True,
                hwp_adapter.export(output_path, hwp_destination),
                None,
                None,
            )
